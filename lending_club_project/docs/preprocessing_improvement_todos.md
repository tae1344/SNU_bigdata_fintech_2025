...
- [ ] `pd.read_csv()` 호출에 `low_memory=False` 추가
- [ ] 데이터 로딩 시 예외 처리 추가
- [ ] 메모리 사용량 모니터링 기능 추가
...
- [ ] `selected_status` 리스트 정의 (Charged Off, Fully Paid 등)
- [ ] `loan_status` 매핑 딕셔너리 생성
- [ ] 기존 `is_default` 로직을 새로운 `target` 로직으로 교체
- [ ] 타겟 변수 분포 시각화 추가
...
- [ ] `id != 'Loans that do not meet the credit policy'` 필터링 추가
- [ ] 제거된 로우 수 기록
- [ ] 제거 전후 데이터 크기 비교 로그 추가
...
- [ ] `clean_percentage_columns()` 함수 생성
- [ ] `int_rate`, `revol_util` 컬럼의 '%' 제거 로직 추가
- [ ] 변환 실패 시 예외 처리 추가
- [ ] 변환 전후 데이터 타입 검증 추가
...
- [ ] `create_fico_features()` 함수 생성
- [ ] FICO 평균값 계산 로직 추가
- [ ] 5점 단위 구간화 로직 구현
- [ ] Ordered Category dtype 변환
- [ ] Ordinal encoding 적용
...
- [ ] `enhanced_categorical_encoding()` 함수 생성
- [ ] sub_grade 순서형 인코딩 구현 (A1→0, G5→34)
- [ ] emp_length 수치화 + 결측 플래그 추가 (`emp_length_is_na`)
- [ ] home_ownership 카테고리 정리 (NONE/ANY → OTHER)
- [ ] 카이제곱 독립성 검정 시스템 구현
- [ ] 범주별 부도율 분석 및 시각화 추가
- [ ] 기존 인코딩 로직과 통합
- [ ] 인코딩 결과 검증 추가
...
- [ ] `optimize_state_encoding()` 함수 생성
- [ ] 상위 99% 주만 유지하는 로직 구현 (51개 → 45개)
- [ ] 나머지는 'OTHER'로 그룹화
- [ ] 변환 전후 카이제곱 검정 비교 분석
- [ ] 주별 부도율 시각화 (원본 vs 축소 버전)
- [ ] 카테고리 타입으로 변환
- [ ] 주별 분포 시각화 추가
...
- [ ] `statistical_validation()` 함수 생성
- [ ] 카이제곱 독립성 검정 자동화
- [ ] 범주별 부도율 분석 함수 구현
- [ ] 이중 축 시각화 함수 생성 (개수 + 부도율)
- [ ] 검정 결과 요약 리포트 생성
- [ ] 모든 범주형 변수에 대한 일괄 검증 시스템
